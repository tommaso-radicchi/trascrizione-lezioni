...
class RepositoryPostgres:
    ...
    def salva(self, lezione: Lezione) -> None:
        with self._connessione.cursor() as cursore:
            cursore.execute(
                """
                INSERT INTO lezioni (
                    id, materia, data, stato, percorso_audio, chat_id, percorso_appunti, tentativi
                )
    ...
    def da_processare(self) -> list[Lezione]:
        with self._connessione.cursor() as cursore:
            cursore.execute(
                """
                SELECT id, materia, data, stato, percorso_audio, chat_id, percorso_appunti, tentativi
                FROM lezioni
...
def _riga_a_lezione(
    riga: tuple[str, str, date, str, str, str, str | None, int],
) -> Lezione:
    id_, materia, data_, stato, percorso_audio, chat_id, percorso_appunti, tentativi = riga
    return Lezione(
        id=id_,
        materia=materia,
        data=data_,
